...
param = [(4, 0, 3), (2, 1, 5), (3, 1, 3), (4, 1, 3), (5, 1, 3), (6, 1, 7), (3, 0, 9), (3, 1, 3)]
...
for i in range(1, 9):
    ...
    ax.scatter(np.arange(100)[outliers], ts[outliers], c='red', s=100, label='ARIMA-Outliers')

    # 箱线图
    q1, q3 = ts.quantile(q=[0.25, 0.75])
    iqr = q3 - q1
    outliers_mask = (ts < (q1 - 1.5 * iqr)) | (ts > (q3 + 1.5 * iqr))
    ax.scatter(np.arange(100)[outliers_mask], ts[outliers_mask], c='green', s=100, label='Boxplot-Outliers')
